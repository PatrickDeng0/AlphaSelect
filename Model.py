...
class Model:
    def __init__(self, mode, learning_rate, activation, optimizer):
        self._mode = mode
        self._learning_rate = learning_rate
        self._activation = activation
        if optimizer == 'RMSprop':
            self._optimizer = tk.optimizers.RMSprop(learning_rate=learning_rate)
        elif optimizer == 'Nadam':
            self._optimizer = tk.optimizers.Nadam(learning_rate=learning_rate)
        else:
            self._optimizer = tk.optimizers.Adam(learning_rate=learning_rate)
        self._model = None
...
class CNN_Pred(Model):
    def __init__(self, mode, input_shape, learning_rate=0.001, num_vr_kernel=64, num_time_kernel=16, num_dense=16,
                 kernel_size=(2, 1), pool_size=(2, 1), strides=(2, 1), activation='relu', optimizer='adam'):
        super().__init__(mode, learning_rate, activation, optimizer)
    ...
    def _build_model(self):
        ...
        x = tk.layers.Dropout(rate=0.2)(x)
...
class LSTM_Model(Model):
    def __init__(self, mode, input_shape, learning_rate=0.001, num_dense=16, activation='relu', optimizer='adam'):
        super().__init__(mode, learning_rate, activation, optimizer)
        self._input_shape = input_shape
        self._num_dense = num_dense
...
class TCN_Model(Model):
    def __init__(self, mode, input_shape, learning_rate=0.001, num_dense=16, activation='relu', optimizer='A'):
        super().__init__(mode, learning_rate, activation, optimizer)
        self._input_shape = input_shape
        self._num_dense = num_dense
